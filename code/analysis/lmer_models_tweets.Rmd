...
model_acc <- lmer(accuracy ~ 1 + belief * truth + belief * truth * party + (1 + belief * truth|author_id), data=tweets_independent, control=lmerControl(optCtrl=list(maxfun=30000, optimizer="bobyqa") ))
```

```{r full model transparency}
model_trans <- lmer(transparency ~ 1 + belief * truth + belief * truth * party + (1 + belief * truth|author_id), data=tweets_independent, control=lmerControl(optCtrl=list(maxfun=30000, algorithm="NLOPT_LN_NELDERMEAD") ))
```

```{r full model word2vec}
model_word2vec <- lmer(NG ~ 1 + belief_word2vec * truth_word2vec + belief_word2vec * truth_word2vec * party + (1 + belief_word2vec * truth_word2vec|author_id), data=tweets_emb, control=lmerControl(optCtrl=list(maxfun=30000),optimizer="bobyqa" ))
```

```{r full model fasttext}
model_fasttext <- lmer(NG ~ 1 + belief_fasttext * truth_fasttext + belief_fasttext * truth_fasttext * party + (1 + belief_fasttext * truth_fasttext|author_id), data=tweets_emb, control=lmerControl(optCtrl=list(maxfun=30000),optimizer="bobyqa" ))
```
